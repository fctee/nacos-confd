...
func IsConfigChanged(src, dest string) (bool, error) {
	...
	if d.Uid != s.Uid {
		log.Info(fmt.Sprintf("%s 的 UID 是 %d，应该是 %d", dest, d.Uid, s.Uid))
	}
	if d.Gid != s.Gid {
		log.Info(fmt.Sprintf("%s 的 GID 是 %d，应该是 %d", dest, d.Gid, s.Gid))
	}
	if d.Mode != s.Mode {
		log.Info(fmt.Sprintf("%s 的模式是 %s，应当是 %s", dest, os.FileMode(d.Mode), os.FileMode(s.Mode)))
	}
	if d.Md5 != s.Md5 {
		log.Info(fmt.Sprintf("%s 的 md5sum 是 %s，应该是 %s", dest, d.Md5, s.Md5))
	}
